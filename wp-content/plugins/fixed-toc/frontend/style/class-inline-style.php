...
class Fixedtoc_Inline_Style {
	 ...
	 * @access private
	 *
	 * @param Fixedtoc_Style_Data $object_data
	 */
	...
	/**
	 * Convert a hex decimal color code to its RGB equivalent
	 *
	 * @since 3.0.0
	 * @access private
	 *
	 ...
	 *
	 * @return array|false|string (depending on second parameter. Returns False if invalid hex color value)
	 */
	public function hex2rgba( $hexStr, $opacity = false, $returnAsString = true, $separator = ',' ) {
		...
		return $returnAsString ? 'rgba(' . implode( $separator, $rgbArray ) . $opacity_suffix . ')' : $rgbArray; // returns the rgb string or the associative array
	}
