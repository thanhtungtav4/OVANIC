...
if ( ! class_exists( 'WPSEO_Video_Analyse_Post' ) ) {
	...
	class WPSEO_Video_Analyse_Post {
		 ...
		 *
		 * {@internal Changing the order of this array will change the priority with which the plugin
		 *            is treated.
		...
		public function __construct( $content, $vid, $old_vid = array(), $post = null ) {
			...
			if ( ! isset( self::$use_embedly ) ) {
				...
				if ( WPSEO_Options::get( 'video_embedly_api_key', '' ) !== '' ) {
		...
		public static function set_statics() {
			...
			foreach ( $supported_plugins as $name => $plugin_basenames ) {
			...
			if ( is_array( self::$active_plugins ) && self::$active_plugins !== array() ) {
				foreach ( self::$active_plugins as $name => $instance ) {
					...
					if ( is_array( $video_autoembeds ) && $video_autoembeds !== array() ) {
						/*
						 * {@internal Merge order reversed, if there is a handler name conflict
						 *            between plugins, defer to the more popular plugin which will
		...
		protected function get_video_from_shortcode() {
			...
			if ( strpos( $this->content, '[' ) !== false && is_array( self::$shortcodes ) && self::$shortcodes !== array() ) {
		 ...
		 *
		 * @return array|bool returns array $urls with type of video as array key and video URL as content, or false on negative
		 */
		protected function grab_embeddable_urls( $content ) {
			...
			if ( $wistia_domain !== '' ) {
				$wistia_info = $this->parse_url( $wistia_domain );
			}
			...
			if ( $url !== '' ) {
				...
				if ( ! isset( $vid['type'] ) || $parsed_url['domainname'] !== $vid['type'] ) {
					...
					if ( ! isset( $wistia_host ) ) {
						...
						$wistia_domain = WPSEO_Options::get( 'video_wistia_domain', '' );
						if ( $wistia_domain ) {
							$wistia_host = 'http://' . $wistia_domain;
		 ...
		 *
		 * {@internal The WP version was introduced in WP 4.4.0 and started supporting
		 *            the second argument "component" in WP 4.7.0.}
		 *
		 ...
		 *
		 * @return array|string|int|false|null False for seriously malformed urls.
		 ...
		 *
		 * {@internal Used to be regex based, but the regex was buggy in a few places, most
		 *            notably it failed on:
		 *            - protocol independent URLs;
		 *            - arguments in array syntax;
		 *            - query starting with `&` not `?`.
		 *
		 *            This version deals with all those cases too and is compatible with the
		 *            expected array return elements from both the old function as well as the
		 *            PHP native parse_url() function.}
		 *
		 * @since    1.1
		 *
		 * @param string     $url       The URL to parse.
