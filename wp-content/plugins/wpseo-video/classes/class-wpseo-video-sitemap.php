...
class WPSEO_Video_Sitemap {
	...
	public function __construct() {
		...
		if ( is_admin() ) {
			...
			elseif ( ! $is_elementor_ajax_save ) {
				...
				\add_action( 'wp_insert_post', array( $this, 'invalidate_sitemap' ), 13 );
			}
	...
	public function is_valid_bot() {
		if ( preg_match( '`(Google|bing)bot`', sanitize_text_field( $_SERVER['HTTP_USER_AGENT'] ), $match ) ) {
			...
			if (
				( $match[1] === 'Google' && preg_match( '`googlebot\.com$`', $hostname )
					&& gethostbyname( $hostname ) === $_SERVER['REMOTE_ADDR'] )
				|| ( $match[1] === 'bing' && preg_match( '`search\.msn\.com$`', $hostname )
					&& gethostbyname( $hostname ) === $_SERVER['REMOTE_ADDR'] )
			) {
				return true;
			}
	...
	public function opengraph() {
		_deprecated_function( __METHOD__, 'WPSEO Video 13.0', 'WPSEO_Video_Abstract_Tag_Presenter' );
	...
	/**
	 * Get the presenters that are applicable to the current page.
	 *
	 * Note: this method can be removed along with `opengraph` when that
	 * deprecated method is removed.
	 *
	 * @return \Abstract_Indexable_Tag_Presenter[] The array of presenters.
	 */
	private function get_presenters() {
		$presenters = [];
		...
		// Only output duration if it is longer than 0 seconds.
		if ( $video['duration'] !== 0 ) {
			$presenters[] = new WPSEO_Video_Duration_Presenter( $video );
		}

		// Only output width and height if both exist.
		...
		// Add Yandex-supported metatags, if enabled. (enabled by default).
