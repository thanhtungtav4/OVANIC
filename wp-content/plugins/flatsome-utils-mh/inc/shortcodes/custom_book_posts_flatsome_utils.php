<?php
// [blog_posts]
function custom_book_posts_flatsome_utils($atts, $content = null, $tag = '' ) {
	...
	extract(shortcode_atts(array(
	    ...
	    'image_depth' => '',
	    'image_depth_hover' => '',
	    'flatsome_ultils_mh_pagination'	=> 'false',
	    'flatsome_ultils_mh_slide_2_row'	=> 'false',
	    'flatsome_ultils_mh_pagination_loadmore'	=> 'false',
	    'flatsome_ultils_mh_pagination_loadmore_label'	=> 'Tải thêm',
	), $atts));
  ...
  if($visibility == 'hidden') return;
	...
	if($cat){
		...
		$args['tax_query'] =  array(
		        array(
		            'taxonomy' => 'book_genre',   // taxonomy name
	...
	if($offset){
		...
		if($latest){
			$args['post__not_in'] = $latest->posts;
		}
	}
...
$atts['tag'] = $tag;
$atts['type'] = $type;
get_flatsome_repeater_start_flatsome_utils($repeater, $atts);
...
if($flatsome_ultils_mh_pagination != 'false' && $flatsome_ultils_mh_pagination != false){
	$atts['is_loadmore'] = $flatsome_ultils_mh_pagination_loadmore != 'false' && $flatsome_ultils_mh_pagination_loadmore != false;
	$atts['flatsome_ultils_mh_pagination_loadmore_label'] = isset($atts['flatsome_ultils_mh_pagination_loadmore_label']) ? $atts['flatsome_ultils_mh_pagination_loadmore_label'] : 'Tải thêm';
	get_flatsome_repeater_end_flatsome_utils($repeater, $recentPosts, $atts);
}
else
	get_flatsome_repeater_end_flatsome_utils($repeater);
...
add_shortcode("custom_book_posts_flatsome_utils", "custom_book_posts_flatsome_utils");
