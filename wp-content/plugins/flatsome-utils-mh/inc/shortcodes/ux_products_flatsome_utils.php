...
// Flatsome Products
function ux_products_flatsome_utils($atts, $content = null, $tag = '' ) {
	$sliderrandomid = rand();
  ...
  if($visibility == 'hidden') return;
	...
	$atts['type'] = $type;
	// debug($type);
	get_flatsome_repeater_start_flatsome_utils($repater, $atts);
	...
	<?php
		...
		if(empty($ids)){
			...
			$products = ux_list_products_flatsome_utils($atts);
    ...
    if($flatsome_ultils_mh_pagination != 'false' && $flatsome_ultils_mh_pagination != false){
		...
		get_flatsome_repeater_end_flatsome_utils($repater, $products, $atts);
    }
	else
		get_flatsome_repeater_end_flatsome_utils($repater);
...
add_shortcode("ux_products_flatsome_utils", "ux_products_flatsome_utils");
