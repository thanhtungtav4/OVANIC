=== Plugin Name ===
Contributors: yithemes
Tags: yith, frequently bought together, bought together, bundles, frequently bought products
Requires at least: 5.7
Tested up to: 6.0
Stable tag: 1.16.0
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

The YITH WooCommerce Frequently Bought Together plugin lets you add product suggestions with a simple summary in each product page.

== Description ==
The right cross selling strategy in your shop is crucial because it affects conversion rates in a very significant way. Products suggested as **“frequently bought together”** are, in fact, much more appealing and customers who see them might be already interested in them. This way, conversion rates are really high.

YITH WooCommerce Frequently Bought Together plugin is the best choice for implementing this feature into your website. You can show products to a highly specific and selected target. Explicitly designed to let you make your offers in the most direct way ever, **a well-planned proposal has double advantages for shop owners and customers**. Your suggestions might allow them to save a lot of time and grant you a highly targeted advertising, which is always the most effective one.

**Main features:**

* Specify an unlimited number of suggested products for each single product of your shop
* Customise the title of the section “Frequently Bought Together”
* Customise text and colours of the button “Add to Cart” placed in the section “Frequently Bought Together”

Full documentation is available [here](https://docs.yithemes.com/yith-woocommerce-frequently-bought-together/).

== Installation ==

1. Unzip the downloaded zip file.
2. Upload the plugin folder into the `wp-content/plugins/` directory of your WordPress site.
3. Activate `YITH WooCommerce Frequently Bought Together` from Plugins page.

== Screenshots ==

1. The plugin admin page.
2. The plugin frontend
3. The plugin tab in edit product

== Changelog ==

== 1.16.0 == Released on 29 July 2022

* New: support for WooCommerce 6.8
* Update: YITH plugin framework

== 1.15.0 == Released on 12 July 2022

* New: support for WooCommerce 6.7
* Update: YITH plugin framework

== 1.14.0 == Released on 23 June 2022

* New: support for WooCommerce 6.6
* Update: YITH plugin framework

== 1.13.0 == Released on 17 May 2022

* New: support for WooCommerce 6.5
* New: support for WordPress 6.0
* Update: YITH plugin framework

== 1.12.0 == Released on 08 April 2022

* New: support for WooCommerce 6.4
* Update: YITH plugin framework

== 1.11.0 == Released on 14March 2022

* New: support for WooCommerce 6.3
* Update: YITH plugin framework

== 1.10.0 == Released on 15 February 2022

* New: support for WooCommerce 6.2
* Update: YITH plugin framework

== 1.9.0 == Released on 18 January 2022
* New: support for WooCommerce 6.1
* New: support for WordPress 5.9
* Update: YITH plugin framework

== 1.8.0 == Released on 20 December 2021
* New: support for WooCommerce 6.0
* Update: YITH plugin framework

== 1.7.0 == Released on 11 November 2021

* New: support for WooCommerce 5.9
* Update: YITH plugin framework

== 1.6.0 == Released on 19 October 2021

* New: support for WooCommerce 5.8
* Update: YITH plugin framework

== 1.5.1 == Released on 27 September 2021

* Update: YITH plugin framework
* Fix: debug info feature removed for all logged in users

== 1.5.0 == Released on 09 September 2021

* New: support for WooCommerce 5.7
* Update: YITH plugin framework

== 1.4.12 == Released on 13 August 2021

* New: support for WooCommerce 5.6
* Update: YITH plugin framework

== 1.4.11 == Released on 14 July 2021

* New: support for WordPress 5.8
* New: support for WooCommerce 5.5
* Update: YITH plugin framework

== 1.4.10 == Released on 07 June 2021

* New: support for WooCommerce 5.4
* Update: YITH plugin framework

== 1.4.9 == Released on 07 May 2021

* New: support for WooCommerce 5.3
* Update: YITH plugin framework

== 1.4.8 == Released on 16 April 2021

* New: support for WooCommerce 5.2
* Update: YITH plugin framework

== 1.4.7 == Released on 09 March 2021

* New: support for WordPress 5.7
* New: support for WooCommerce 5.1
* Update: YITH plugin framework

== 1.4.6 == Released on 05 February 2021

* New: support for WooCommerce 5.0
* Update: YITH plugin framework
* Fix: undefined index 'offeringID'

== 1.4.5 == Released on 13 January 2021

* New: Support for WooCommerce 4.9
* Update: Plugin framework

== 1.4.4 == Released on 04 December 2020

* New: Support for WooCommerce 4.8
* Update: Plugin framework

== 1.4.3 == Released on 06 November 2020

* New: Support for WooCommerce 4.7
* New: Support for WordPress 5.6
* Update: Plugin framework

== 1.4.2 == Released on 12 October 2020

* New: Support for WooCommerce 4.6
* Update: Plugin framework

== 1.4.1 == Released on 17 September 2020

* New: Support for WooCommerce 4.5
* Update: Plugin framework
* Fix: Missing default value for plugin options

== 1.4.0 == Released on 26 August 2020

* New: Support for WooCommerce 4.4
* New: Support for WordPress 5.5
* Update: Plugin framework

== 1.3.2 == Released on 01 June 2020

* New: Support for WooCommerce 4.2
* Update: Plugin framework
* Fix: Fatal error call is_purchasable() on null

== 1.3.1 == Released on 23 April 2020

* New: Support for WooCommerce 4.1
* Update: Plugin framework

== 1.3.0 == Released on 09 March 2020

* New: Support for WooCommerce 4.0
* New: Support for WordPress 5.4
* Update: Plugin framework

== 1.2.16 == Released on 21 December 2019

* New: Support for WooCommerce 3.9
* New: Support for WordPress 5.3.2
* Update: Plugin framework

== 1.2.15 == Released on 29 November 2019

* Update: Plugin framework

== 1.2.14 == Released on 05 November 2019

* Update: Plugin framework

== 1.2.13 == Released on 05 November 2019

* Update: Plugin framework

== 1.2.12 == Released on 30 October 2019

* Update: Plugin framework

== 1.2.11 == Released on 29 October 2019

* New: Support for WooCommerce 3.8
* New: Support for WordPress 5.3
* Update: Plugin framework

== 1.2.10 == Released on 01 August 2019

* New: Support to WooCommerce 3.7.0 RC1
* New: Support to WordPress 5.2.2
* New: Plugin admin panel style
* Update: Plugin core
* Fix: Do not add out of stock products to frequently bought group

== 1.2.9 == Released on 13 June 2019

* New: Support to WooCommerce 3.6.4
* New: Support to WordPress 5.2.1
* Update: Plugin core

== 1.2.8 == Released on 23 April 2019

* New: Support to WooCommerce 3.6.1
* Update: Plugin core

== 1.2.7 == Released on 09 April 2019

* New: Support to WooCommerce 3.6.0 RC1
* Update: Plugin core

== 1.2.6 == Released on 30 January 2019

* New: Support to WooCommerce 3.5.4
* Update: Plugin core

== 1.2.5 == Released on 06 December 2018

* New: Support to WooCommerce 3.5.2
* New: Support to WordPress 5.0.0
* New: Support to Gutenberg
* Update: Plugin core

== 1.2.4 == Released on 23 October 2018

* New: Support to WooCommerce 3.5.0
* Update: Plugin core

== 1.2.3 == Released on 10 September 2018

* New: Support to WooCommerce 3.5.0 RC1
* Update: Plugin core

== 1.2.2 == Released on 10 September 2018

* New: Support to WordPress 4.9.8
* New: Support to WooCommerce 3.4.5
* Update: Plugin core
* Dev: New action "yith_wfbt_group_added_to_cart" after add to cart process

== 1.2.1 == Released on 31 May 2018

* New: Support to WordPress 4.9.6
* New: Support to WooCommerce 3.4.1
* Update: Plugin core
* Fix: Documentation url

== 1.2.0 == Released on 01 February 2018

* New: Support to WordPress 4.9.2
* New: Support to WooCommerce 3.3.0
* Update: Plugin core
* Fix: Shortcode [ywfbt_form] now use global product if no id is passed as argument
* Dev: New filter yith_wfbt_product_types_meta_save to add custom product types to plugin admin actions

== 1.1.3 == Released on 17 October 2017

* New: Support to WordPress 4.8.2
* New: Support to WooCommerce 3.2.1
* New: Danish Translation
* Update: Plugin core

== 1.1.2 == Released on 22 July 2017

* New: Support to WordPress 4.8
* New: Support to WooCommerce 3.1.1
* Update: Plugin core
* Fix: Exclude deleted products automatically from frequently bought products

== 1.1.1 == Released on 24 April 2017

* New: Support to WooCommerce 3.0.4
* New: Support to WordPress 4.7.4
* Update: Plugin core
* Fix: Prevent multiple postmeta entry for same product

== 1.1.0 == Released on 28 March 2017

* New: Support to WooCommerce 3.0.0 RC2
* New: Support to WordPress 4.7.3
* Update: Plugin core

== 1.0.7 == Released on 26 October 2016

* New: Support to WooCommerce 2.6.6
* New: Support to WordPress 4.6.1
* New: Shortcode [ywfbt_form product_id=""] to print form "bought together" products
* Update: Plugin core

== 1.0.6 == Released on 16 August 2016

* New: Support to WooCommerce 2.6.4
* New: Support to WordPress 4.6
* Update: Plugin core

== 1.0.5 == Released on 10 June 2016

* New: Support to WooCommerce 2.6 RC1
* Update: Plugin core

== 1.0.4 == Released on 27 April 2016

* New: WordPress 4.5 compatibility
* Update: Template yith-wfbt-form.php to version 1.0.4
* Update: Changed plugin textdomain from ywbt to yith-woocommerce-frequently-bought-together
* Update: Language file
* Update: Plugin core

== 1.0.3 == Released on 12 January 2016

* New: Compatibility with WooCommerce 2.5 RC2
* Update: Plugin Core

== 1.0.2 == Released on 21 August 2015

* New: Compatibility with WooCommerce 2.4.x
* New: Compatibility with WordPress 4.3
* New: ITA Translation
* Update: Plugin Core

== 1.0.1 == Released on 24 July 2015

* Initial release

== Suggestions ==

If you have suggestions about how to improve YITH WooCommerce Frequently Bought Together, you can [write us](mailto:plugins@yithemes.com "Your Inspiration Themes") so we can bundle them into YITH WooCommerce Frequently Bought Together.

== Translators ==

= Available Languages =

* English
* Italian
* Spanish
* Danish

If you can't find this plugin in your language and want to translate it, use the dedicated tool at [Translating WordPress](https://translate.wordpress.org/locale/it/default/wp-plugins/yith-woocommerce-frequently-bought-together "Translating WordPress").
Or, if you have created your own language pack, or have an update for an existing one, you can send it as an email attachment from our [Contacts page](http://yithemes.com/contact/ "Your Inspiration Themes") and we will bundle into YITH WooCommerce Frequently Bought Together.

== Upgrade notice ==

== 1.5.1 == Released on 27 Sep 2021

* Update: YITH plugin framework
* Fix: debug info feature removed for all logged in users
