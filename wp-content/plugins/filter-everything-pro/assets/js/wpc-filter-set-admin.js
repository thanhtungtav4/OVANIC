/*!
 * Filter Everything set admin 1.6.3
 */
(function($) {
    "use strict";
    ...
    $(document).ready(function (){
        ...
        $('.wpc-add-filter').on('click', function (e){
            ...
            let applyButtonPosition  = $("#wpc_set_fields-apply_button_menu_order").val();
            let filtersListContainer = $('#wpc-filters-list');
            let filtersLength        = filtersListContainer.find(".wpc-filter-item").length.toString();
            let applyButtonItem      = $("#wpc-filter-id-apply-button");
            ...
            if( $(".wpc-filter-item:not(.wpc-filter-item-apply-button)").length === 1 ){
        ...
        $('body').on('click', '#wpc_set_fields-use_apply_button', function (){
            ...
            if( applyButtonChecked ){
                ...
                if( $(".wpc-filter-item:not(#wpc-filter-id-apply-button)").length < 1 ){
                    $('.wpc-no-filters').show();
                }
            }
        });
        ...
        if( applyButtonPosition === '-1' ){
            let filtersLength = $('#wpc-filters-list').find(".wpc-filter-item").length.toString();
            $("#wpc_set_fields-apply_button_menu_order").val(filtersLength);
        }
    ...
    function wpcAddNewFilterToParentList(){
        ...
        $(".wpc-filter-item:not(.wpc-filter-item-apply-button)").each( function ( index, elem ){
    ...
    {
        if( $(".wpc-filter-item:not(#wpc-filter-id-apply-button)").length > 0 ){
            $('.wpc-no-filters').hide();
        }else{
            $('.wpc-no-filters').show();
        }
