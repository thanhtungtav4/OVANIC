...
<input type="hidden" name="woof_settings[<?php esc_attr_e($key) ?>][options]" value="<?php esc_html_e(isset($settings[$key]['options']) ? $settings[$key]['options'] : "") ?>" /> 
<input type="hidden" name="woof_settings[<?php esc_attr_e($key) ?>][search_logic]" value="<?php esc_html_e(isset($settings[$key]['search_logic']) ? $settings[$key]['search_logic'] : "OR") ?>" /> 
<div id="woof-modal-content-<?php esc_attr_e($key) ?>" style="display: none;">
    ...
    <div class="woof-form-element-container">
        <div class="woof-name-description">
            <strong><?php esc_html_e('Search logic', 'woocommerce-products-filter') ?></strong>
            <span><?php esc_html_e('Check search logic AND/OR', 'woocommerce-products-filter') ?></span>
        </div>

        <div class="woof-form-element">
            <?php
            $logic = array(
                'OR' => esc_html__('OR', 'woocommerce-products-filter'),
                'AND' => esc_html__('AND', 'woocommerce-products-filter')
            );
            ?>

            <div class="select-wrap">
                <select class="woof_popup_option" data-option="search_logic">
                    <?php foreach ($logic as $id => $value) : ?>
                        <option value="<?php esc_attr_e($id) ?>"><?php esc_html_e($value) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
