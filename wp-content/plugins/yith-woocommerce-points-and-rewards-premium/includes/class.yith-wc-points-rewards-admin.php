...
if ( ! class_exists( 'YITH_WC_Points_Rewards_Admin' ) ) {
	...
	class YITH_WC_Points_Rewards_Admin {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_ywpar_dismiss_notice_message', array( $this, 'ajax_dismiss_notice' ) );
		...
		public function check_coupon() {
			...
			if ( isset( $_GET['page']) && 'yith_woocommerce_points_and_rewards' === $_GET['page'] ) { //phpcs:ignore
				if ( 'yes' !== get_option( 'woocommerce_enable_coupons' ) && 'yes' !== get_option( 'ywpar_dismiss_disabled_coupons_warning_message', 'no' ) ) { ?>
					<div id="message" class="notice notice-warning is-dismissible ywpar_disabled_coupons">
						<p>
					...
					<script>
						( function ( $ ) {
							$( '.ywpar_disabled_coupons' ).on( 'click', '.notice-dismiss', function () {
								jQuery.post( "<?php echo esc_html( admin_url( 'admin-ajax.php' ) ); ?>", {
									action        : "ywpar_dismiss_notice_message",
									dismiss_action: "ywpar_dismiss_disabled_coupons_warning_message",
									nonce         : "<?php echo esc_js( wp_create_nonce( 'ywpar_dismiss_notice' ) ); ?>"
								} );
							} );
						} )( jQuery );
					</script>
		...
		public function ajax_dismiss_notice() {
			...
			check_ajax_referer( 'ywpar_dismiss_notice', 'nonce' );
			switch ( $_POST['dismiss_action'] ) {
				case 'ywpar_dismiss_disabled_coupons_warning_message':
					update_option( 'ywpar_dismiss_disabled_coupons_warning_message', 'yes' );
					break;
			}
			wp_die();
		}
