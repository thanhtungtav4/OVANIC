	...
	class YITH_WC_Points_Rewards_Redemption {
		...
		public function deduce_order_points( $order ) {
			...
			if ( $customer_user ) {
				...
				$order->add_order_note( sprintf( __( '%1$d %2$s to get a reward', 'yith-woocommerce-points-and-rewards' ), - $points, YITH_WC_Points_Rewards()->get_option( 'points_label_plural' ) ), 0 );
			}
		...
		public function get_user_rewarded_points( $user_id ) {
			...
			$query           = "SELECT SUM(pl.amount) FROM $table_name as pl where pl.user_id = $user_id AND ( pl.action IN ( 'redeemed_points', 'order_refund', 'admin_action', 'order_cancelled') AND pl.amount < 0 )";
		...
		public function rewards_order_points( $order ) {
			...
			if ( ! empty( $points_redemption ) ) {
				...
				if ( $customer_id ) {
					...
					$order->add_order_note( sprintf( __( '%1$d %2$s to get a reward', 'yith-woocommerce-points-and-rewards' ), - $points_redemption, YITH_WC_Points_Rewards()->get_option( 'points_label_plural' ) ), 0 );
