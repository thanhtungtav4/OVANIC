...
if ( ! class_exists( 'YITH_WC_Points_Rewards_Earning' ) ) {
	...
	class YITH_WC_Points_Rewards_Earning {
		...
		public function init() {
			...
			if ( YITH_WC_Points_Rewards()->get_option( 'assign_older_orders_points_to_new_registered_user' ) == 'yes' ) {
		...
		public function add_points_for_previous_orders_on_registration( $user_id ) {
			...
			if ( $eventual_orders ) {
				foreach ( $eventual_orders as $o ) {
					if ( $o->get_meta( 'ywpar_points_from_cart' ) ) {
						$total_points_to_add += $o->get_meta( 'ywpar_points_from_cart' );
					} else {
						$subtotal      = $o->get_subtotal() + $o->get_cart_tax();
						$points_to_add = YITH_WC_Points_Rewards_Earning()->get_point_earned_from_price( $subtotal, false, '' );
		...
		public function add_order_points( $order_id ) {
			...
			$order->add_order_note( sprintf( __( 'Customer earned %1$d %2$s for this purchase.', 'yith-woocommerce-points-and-rewards' ), $tot_points, YITH_WC_Points_Rewards()->get_option( 'points_label_plural' ) ), 0 );
		...
		public function remove_order_points( $order_id ) {
			...
			if ( $customer_user > 0 ) {
				...
				$order->add_order_note( sprintf( __( 'Removed %1$d %2$s for order %3$s.', 'yith-woocommerce-points-and-rewards' ), - $points, YITH_WC_Points_Rewards()->get_option( 'points_label_plural' ), YITH_WC_Points_Rewards()->get_action_label( $action ) ), 0 );
			}
		...
		public function refund_delete() {
			...
			if ( $refund_amount > 0 ) {
				...
				if ( $user_id > 0 ) {
					...
					$order->add_order_note( sprintf( __( 'Added %1$d %2$s for cancelled refund.', 'yith-woocommerce-points-and-rewards' ), $points, YITH_WC_Points_Rewards()->get_option( 'points_label_plural' ) ), 0 );
				}
			}
		...
		public function remove_order_points_refund( $order_id, $refund_id ) {
			...
			if ( $refund_amount > 0 ) {
				...
				if ( $user_id > 0 ) {
					...
					YITH_WC_Points_Rewards()->add_point_to_customer( $user_id, - $points, $action, '', $order_id );
					$order->add_order_note( sprintf( __( 'Removed %1$d %2$s for order refund.', 'yith-woocommerce-points-and-rewards' ), $points, YITH_WC_Points_Rewards()->get_option( 'points_label_plural' ) ), 0 );
				}
			}
		}
